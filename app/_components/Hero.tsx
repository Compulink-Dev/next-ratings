...
function Hero() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch categcolorories");
        }
  ...
  const handleSearch = () => {
    if (selectedCategory) {
      ...
      if (category) {
        router.push(
          `/categories/${category._id}?name=${encodeURIComponent(
            category.name
          )}`
        );
      }
      ...
      router.push(
        `/companies?search=${encodeURIComponent(searchInput.trim())}`
      );
    }
  ...
  return (
    <div className="relative h-[700px] lg:h-[800px] pt-[100px] bg-[url('/images/hero.jpg')] bg-top bg-no-repeat bg-fixed flex items-center justify-center">
